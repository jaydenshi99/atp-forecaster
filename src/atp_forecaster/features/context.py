...
def get_data():
    ...
    df2 = pd.read_parquet("./data/features/glicko2_ratings.parquet")
...
def main():
    ...
    hth_df.to_parquet("./data/features/head_to_head.parquet", index=True)
    logger.info(f"Head to head feature calculation completed! Saved {len(hth_df)} rows to ./data/features/head_to_head.parquet")

    exp_df = experience(df)
    exp_df.to_parquet("./data/features/experience.parquet", index=True)
    logger.info(f"Experience feature calculation completed! Saved {len(exp_df)} rows to ./data/features/experience.parquet")


    mom_df = momentum_features(df)
    mom_df.to_parquet("./data/features/momentum.parquet", index=True)
    logger.info(f"Momentum feature calculation completed! Saved {len(mom_df)} rows to ./data/features/momentum.parquet")

    fat_df = fatigue_features(df)
    fat_df.to_parquet("./data/features/fatigue.parquet", index=True)
    logger.info(f"Fatigue feature calculation completed! Saved {len(fat_df)} rows to ./data/features/fatigue.parquet")
